...
export const login = (username, password) => {
    ...
    if (result) {
        ...
        // Ensure only shreehari users can login through this service
        if (user && user.hotelId === 'shreehari') {
            return true;
        } else {
            sharedLogout();
            return false;
...
export const isAuthenticated = () => {
    ...
    if (isAuth) {
        const user = sharedGetCurrentUser();
        // Only return true if user belongs to shreehari
        return user && user.hotelId === 'shreehari';
    }
    return false;
};
...
export const getCurrentUser = () => {
    const user = sharedGetCurrentUser();
    // Only return user if they belong to shreehari
    if (user && user.hotelId === 'shreehari') {
        return user;
    }
    return null;
...
export const getCurrentHotelId = () => {
    ...
    // Only return hotel ID if it's shreehari
    return hotelId === 'shreehari' ? hotelId : null;
};